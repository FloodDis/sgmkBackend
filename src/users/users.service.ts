...
export class UsersService {
  ...
  async updateUser(id: number, dto: CreateUserDto) {
    ...
    const photo = dto.photo_id
      ? await this.fileService.getFileById(dto.photo_id)
      : await this.fileService.createFile(dto.photo);
    user.photo = photo;

    const resume = dto.resume_id
      ? await this.resumeService.getResumeById(dto.resume_id)
      : await this.resumeService.createResume(dto.resume);
    user.resume = resume;
