...
export class ResumeService {
  ...
  async createResume(dto: CreateResumeDto) {
    ...
    resume.file = dto.file_id
      ? await this.fileService.getFileById(dto.file_id)
      : await this.fileService.createFile(dto.file);
  ...
  async getAllResumes() {
    ...
    return resumes.map((x) => new ResumeResponseDto(x));
  }
}
